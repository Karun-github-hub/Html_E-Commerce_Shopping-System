...
<div class="wrap-header-cart js-panel-cart">
		...
		<div class="container">
			<div class="flex-w flex-sb-m p-b-52">
				<div class="flex-w flex-l-m filter-tope-group m-tb-10">
					...
					<button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".crop">
						Crop
					</button>
					...
					<button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".Skirt">
						Skirt
					</button>

					<button class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".Coat">
						Coat
					</button>
				</div>
			...
			<?php
				...
				$sql = "SELECT * FROM products p ,product_category c where p.PRDCT_ID = c.PRDCT_ID ";
				$result = $conn->query($sql);

				if ($result->num_rows > 0) {
    			// Her bir satır için verileri alıp gösterme
    			while($row = $result->fetch_assoc()) {
					$productID = $row["PRDCT_ID"];
        		// HTML şablonunu kullanarak verileri gösterme
        		echo '	<div class="col-sm-6 col-md-4 col-lg-3 p-b-35 isotope-item ' . $row["CATEGORY_NAME"] .'">
                <div class="block2">
                    <div class="block2-pic hov-img0">
					<img class="product-image" style="max-width: 100%; height: auto; display: block; margin: 0 auto;" src="images/' .$row["PRDCT_IMG"]. '" alt="Product Image">
					<a href="selectedProduct.php?prdct_id=' . (int)$row["PRDCT_ID"] . '&usr_id=' . (int)$USR_ID . '"  class="block2-btn flex-c-m stext-103 cl2 size-102 bg0 bor2 hov-btn1 p-lr-15 trans-04">Quick View</a>
                    </div>
                    <div class="block2-txt flex-w flex-t p-t-14">
                        <div class="block2-txt-child1 flex-col-l ">
                            <a href="#" class="stext-104 cl4 hov-cl1 trans-04 js-name-b2 p-b-6">' . $row["PRDCT_NAME"] . '</a>
                            <span class="stext-105 cl3">$' . $row["UNIT_PRICE"] . '</span>
                        </div>
                        <div class="block2-txt-child2 flex-r p-t-3">
						<a href="#" class="btn-addwish-b2 dis-block pos-relative js-addwish-b2" onclick="addToFavorites(' . $row["PRDCT_ID"] . ', ' . $USR_ID . ')">
	...
	<script>
function addToFavorites(productID, userID) {
    // AJAX ile veritabanına ekleme işlemi
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            // Başarılı işlem sonrasında isteğe bağlı olarak bir şeyler yapabilirsiniz.
            console.log(this.responseText);
        }
    };
    xhttp.open("POST", "addToFavorites.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send("productID=" + productID + "&userID=" + userID);
}
</script>
